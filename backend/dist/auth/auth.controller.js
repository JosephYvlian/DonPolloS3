...
let AuthController = class AuthController {
    ...
    async register(body) {
        ...
        if (!/^\d+$/.test(body.telefono)) {
            throw new common_1.BadRequestException('El campo teléfono solo debe contener números');
        }
        const passwordRegex = /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[\W_]).{8,}$/;
        if (!passwordRegex.test(body.passwordHash)) {
            throw new common_1.BadRequestException('La contraseña debe tener al menos 8 caracteres, una mayúscula, una minúscula, un número y un carácter especial.');
        }
        return this.authService.register(body);
    }
    async login(body) {
        if (!body.correo || !body.password) {
            throw new common_1.BadRequestException('Correo y contraseña requeridos');
        }
